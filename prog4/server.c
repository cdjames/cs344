...
int main(int argc, char *argv[])
{
 ...
 //    // setsockopt(listenSocketFD, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int));
	...
	// int listening;
	// listening = listen(listenSocketFD, maxConnections); // Flip the socket on - it can now receive up to 5 connections
	// if(listening < 0)
        			...
        			if(i == listenSocketFD){ // there is a new listener
						...
						if(numConnections < maxConnections) {
							...
							if (establishedConnectionFD < 0) 
								...
								/* don't want to accept any more connections */
								if(numConnections >= maxConnections){
									close(listenSocketFD); // close to reject new connections
									FD_CLR(i, &master); // remove the listening socket from the set
								}
							}
	        			}
	        				...
	        				if(printerr > 0){
		        				printf("server busy with %d connections. Try again later.\n", numConnections);
								printerr = 0;
							}
	        			}
        			}
						...
						/* check for need to restart socket and error printing */
						if(numConnections < maxConnections){
							printerr = 1;
							/* check whether socket needs to be reopened */
